    ...
    if file_buffer is not None:
        ...
        if st.button("START GEN"):
            for index, value in enumerate(checked_df.values):
                ...
                char_spacing, min_size, max_size, min_text_len, max_text_len, random_color = value[:-4][6:]
                ...
                if opt.method == 'white':
                    ...
                    runner.run()
                st.text("Time for this process was %s seconds" % int(time.time() - begin_time))


def check_valid(dataframe, bg_df, source_df, fonts_df):
    """
    Check if the valid of input config
    """
    df = dataframe.copy()
    results = {}
